param(
    ...
    [string]$Command = "help",
    [string]$ContainerName = "ollama-mistral",
    [int]$Port = 11435
)
...
function Start-Container {
    ...
    if ($existing -eq $ContainerName) {
        ...
        Write-Host "[1/2] Starting Mistral..." -ForegroundColor Cyan
...
function Pull-OllamaModel {
    Write-Host "Pulling Mistral..." -ForegroundColor Cyan
    docker exec $ContainerName ollama pull $ModelName
    Write-Host "[OK] Mistral ready" -ForegroundColor Green
}

function Test-Quick {
    Write-Host "Quick test with Mistral..." -ForegroundColor Cyan
...
function Show-Status {
    Write-Host "`n=== Mistral Status ===" -ForegroundColor Cyan
...
function Show-Help {
    Write-Host "`n=== Mistral 7B - PowerShell Script ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  pull        - Pull Mistral model"
