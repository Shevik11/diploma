param(
    ...
    [string]$Command = "help",
    [string]$ContainerName = "ollama-llama",
    [int]$Port = 11434
)

# Preconfigured for Llama 2
$ModelName = "llama2"
...
function Start-Container {
    ...
    if ($existing -eq $ContainerName) {
        ...
        Write-Host "[1/2] Starting Llama..." -ForegroundColor Cyan
...
function Pull-OllamaModel {
    Write-Host "Pulling Llama 2..." -ForegroundColor Cyan
    docker exec $ContainerName ollama pull $ModelName
    Write-Host "[OK] Llama 2 ready" -ForegroundColor Green
}

function Test-Quick {
    Write-Host "Quick test with Llama 2..." -ForegroundColor Cyan
...
function Show-Status {
    Write-Host "`n=== Llama 2 Status ===" -ForegroundColor Cyan
...
function Run-Performance-Test {
    ...
    Write-Host "Running automated performance tests..." -ForegroundColor Yellow
    python scripts\tests\performance_test.py $ModelName $Port
}

function Run-Quality-Test {
    Write-Host "`n=== Quality Assessment ===" -ForegroundColor Cyan
    Write-Host "Running quality tests..." -ForegroundColor Yellow
    python scripts\tests\quality_test.py $ModelName $Port
}

function Show-Help {
    Write-Host "`n=== Llama 2 - PowerShell Script ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  pull         - Pull Llama 2 model"
    ...
    Write-Host "  quality      - Run quality assessment"
    ...
    Write-Host "  .\make.ps1 benchmark"
    Write-Host "  .\make.ps1 quality"
    Write-Host ""
}
