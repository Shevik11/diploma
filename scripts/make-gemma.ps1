param(
    ...
    [string]$Command = "help",
    [string]$ContainerName = "ollama-gemma",
    [int]$Port = 11436
)

# Preconfigured for Gemma
$ModelName = "gemma:2b"
...
function Start-Container {
    ...
    if ($existing -eq $ContainerName) {
        ...
        Write-Host "[1/2] Starting Gemma..." -ForegroundColor Cyan
...
function Pull-OllamaModel {
    Write-Host "Pulling Gemma..." -ForegroundColor Cyan
    docker exec $ContainerName ollama pull $ModelName
    Write-Host "[OK] Gemma ready" -ForegroundColor Green
}

function Test-Quick {
    Write-Host "Quick test with Gemma..." -ForegroundColor Cyan
...
function Show-Status {
    Write-Host "`n=== Gemma Status ===" -ForegroundColor Cyan
...
function Show-Help {
    Write-Host "`n=== Gemma 2B - PowerShell Script ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  pull        - Pull Gemma model"
